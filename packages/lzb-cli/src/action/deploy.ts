...
export const handleDeploy = async ({}: Options) => {
    ...
    // 1. 选择发布的项目
        ...
        let monorepoProject = '';
        if (target.type === 'monorepo'){
            // 如果是 monorepo
            monorepoProject = (
                await inquirer.prompt({
                    type: 'list',
                    name: 'monorepoProject',
                    message: '请选择monorepo项目',
                    choices: Object.keys(target.monorepoProject).map((t) => ({name: t, value: target.monorepoProject[t]}))
                })
            ).monorepoProject;
        }
        ...
        const env = (
            await inquirer.prompt({
                ...
                name: 'env',
                message: '请选择发布的环境',
                choices: [{name:'测试',value:'test'},{name:'线上',value:'online'}]
            })
        ).env;
        if (env === 'online') {
            // 2. 是否发布线上
        ...
        if (TASK[project].type === 'uni-app') {
            ...
            await pureWeb(project, env, token,monorepoProject);
        }
...
const pureWeb = (project: string, env: string, token: string,monorepoProject:string) => {
    ...
    // 当前环境的任务
        ...
        for (let index = 0; index < currentTask.length; index++) {
            ...
            const cp = exec(`curl -X POST ${HOST}${task}${monorepoProject? '?monorepoName='+monorepoProject :'' } --user ${token}`);
            // 失败时，stdout 会输出
            cp.stdout?.on('data', () => {
